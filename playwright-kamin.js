...
const RESPONSE_DELAY_MS = 50; // was 300 — just enough for DOM stability
...
/**
 * Press a key targeted at #tapTap input (for stimuli trials using buttonPress).
 * Dispatches keydown + keyup on the element with proper keyCode.
 */
async function pressTrialKey(page, keyChar, delay = RESPONSE_DELAY_MS) {
  ...
  await page.evaluate(
    ({ keyChar, keyCode }) => {
      ...
      // Focus the hidden input if it exists
...
async function handleSurvey(page, choice = SURVEY_CHOICE) {
  ...
  if (typeof choice === "string") {
    ...
    if (byValue) {
      await byValue.click();
      clicked = true;
      console.log(`    [survey] Clicked radio by value="${choice}"`);
    }
  }
  ...
  if (!clicked) {
    ...
    if (idx < radios.length) {
      ...
      clicked = true;
      console.log(`    [survey] Clicked radio by index=${idx} (of ${radios.length})`);
    }
  }
